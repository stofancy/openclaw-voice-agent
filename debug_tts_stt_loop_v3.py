    ...
    try:
        ...
        if result:
            ...
            if audio_bytes[:4] == b'RIFF':
                print(f"✅ TTS 成功! 音频大小: {len(audio_bytes)} 字节")
...
def test_stt_official(pcm_path):
    """使用官方示例代码测试 STT - 收集所有 text 事件"""
    print("\n" + "=" * 60)
    print("步骤 2: 测试 STT 服务 (修复实时转录版)")
    print("=" * 60)
        ...
        try:
            ...
            if event_type == "conversation.item.input_audio_transcription.text":
                ...
                if transcript:
                    ...
                    print(f"📝 转录片段 [{len(transcript_parts)}]: '{transcript}'")
    ...
    def on_close(ws, close_status_code, close_msg):
        ...
        # 连接关闭时，如果有转录片段，返回最后的完整结果
        if transcript_parts:
            final_transcript = ''.join(transcript_parts)
            print(f"📤 返回累积转录: '{final_transcript}'")
...
def compare_results(original_text, stt_result):
    ...
    # 检查是否包含原始文本
    if original_text in stt_result:
        print("\n✅ 测试通过! STT 成功识别了 TTS 生成的语音")
        return True
    else:
        # 检查关键词匹配
        keywords = ["北京", "天气", "查询"]
        matched = [k for k in keywords if k in stt_result]
        if matched:
            print(f"\n⚠️ 部分匹配: 匹配到关键词 {matched}")
            return False
        else:
            print("\n❌ 测试失败: 无法识别")
            return False
